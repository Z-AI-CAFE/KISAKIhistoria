...
'神聖ローマ','ハプスブルク','イーストリア','エルサレム','ナバラ','ナポリ',
'イギラス','ポルトガル','ベルギー','ネーデルラント','オランダ','プロイセン','ドイツ'];
...
(seeAlsoHtml ? '<div class="ei-related"><span class="label">関連：</spam>'+seeAlsoHtml+'</div>' : '');
...
<<<<<<< HEAD
// 家系図または関連動画ありの場合は detail.html へ、それ以外はYouTubeへ直接
if(item.has_family_tree || (item.related_videos && item.related_videos.length)){
=======
// 家系図ありの場合は detail.html へ、それ以外はYouTubeへ直接
if(item.has_family_tree){
>>>>>>> 3821e9ed0313f7693750d3bc46449d912029bbca
...
'<span class="play">'+(item.has_family_tree?'家系図＋動画で学ぶ':(item.related_videos&&item.related_videos.length?'関連動画付きで学ぶ':'解説動画で詳しく学ぶ'))+'</span>';
cards.appendChild(a);
}
}
...
render();
if(currentQuery) scrollToEventSection(); // 事件カード・人物一覧へスクロール
});
});
...
// 事件ドロップダウン（各selectはbuildEventDropdown内で個別にリスナー登録済み）
